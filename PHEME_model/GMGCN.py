...
class GMGCN(torch.nn.Module):
    def __init__(self,user_in_feats,user_hid_feats,user_out_feats, group_in_feats,group_hid_feats, group_out_feats,att_feats, kernel_size):
        super(GMGCN, self).__init__()
        self.conv1 = GroupEmbedding(user_in_feats,user_hid_feats,user_out_feats, group_in_feats,att_feats, kernel_size)
        self.fc1 = torch.nn.Linear(group_in_feats, group_hid_feats)
        self.fc2 = torch.nn.Linear(group_hid_feats, group_out_feats)
    ...
    def forward(self,dev,train_loader,group_edge_index):
        ...
        if self.type=='W':
            out_x = self.conv2(group_x, group_edge_index)

        else:
            out_x = self.fc2(self.fc1(group_x))
        out_labels = F.log_softmax(out_x, dim=1)
        return out_labels,group_x,group_y
